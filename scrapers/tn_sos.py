...
def _search_one(page, name: str) -> list[dict]:
    ...
    try:
        ...
        if not results:
            for card in soup.find_all(attrs={"class": re.compile(r"card|result|entity|row", re.I)}):
                text = card.get_text(" ", strip=True)
                if not text or len(text) < 5:
                    continue
                heading = card.find("h2") or card.find("h3") or card.find("strong")
                if not heading:
                    continue
                link = card.find("a")
                entity = {"registered_name": heading.get_text(strip=True), "detail_url": ""}
                if link and link.get("href"):
                    href = link["href"]
                    entity["detail_url"] = (
                        "https://tncab.tnsos.gov" + href
...
def enrich_with_sos(clinics: list[dict]) -> list[dict]:
    """Looks up each clinic in TN SOS TNCaB and adds ownership + entity info."""
    with sync_playwright() as pw:
        ...
        for i, clinic in enumerate(clinics):
            ...
            if best:
                clinic["registered_name"] = best.get("registered_name", "")
                clinic["entity_type"] = best.get("entity_type", "")
                clinic["sos_status"] = best.get("sos_status", "")
                clinic["sos_id"] = best.get("sos_id", "")

                if best.get("detail_url"):
                    time.sleep(random.uniform(1.0, 2.0))
                    owners = _get_owners(page, best["detail_url"])
                    clinic["owners_officers"] = owners
                    if owners:
                        print(f"    -> {owners[:80]}")
            else:
                clinic["owners_officers"] = ""
                clinic["sos_status"] = "Not Found in TN SOS"
